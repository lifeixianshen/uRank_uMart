...
def convert(tfrecords_folder, file_type, fold):
    ...
    complete_file_name = os.path.join(
        MODIFIED_RANK_DATA, tfrecords_folder, fold, f"{full_file_name}.txt"
    )
    if tfrecords_folder == 'OHSUMED':
        complete_file_name = os.path.join(
            MODIFIED_RANK_DATA,
            ...
            'Feature-min',
            fold,
            f"{full_file_name}.txt",
        )

    with open(complete_file_name, 'w') as writer:
        max_height = 0
        ...
        for group in group_features:
            ...
            for i in range(0, len(normalized_feature_array)):
                ...
                feature_string = [f' {i}:{r[i - 1]}' for i in range(1, len(r) + 1)]
                feature_string = ' '.join(feature_string)
                feature_string = f'{label} qid:{qid}{feature_string}'
